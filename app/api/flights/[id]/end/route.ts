...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data, error } = await supabase.rpc("end_flight", {
      flight_uuid: params.id,
      pilot_profile_uuid: pilotProfile.id,
    })

    if (error) {
      console.error("[v0] End flight error:", error)
      return NextResponse.json({ error: error.message || "Failed to end flight" }, { status: 400 })
    }
